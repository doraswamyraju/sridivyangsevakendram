...
import { 
  ...
  Trash2, Upload, Newspaper, Plus, CheckCircle, Clock 
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50">
      ...
      {/* --- SIDEBAR --- */}
      <aside className="w-64 bg-blue-900 text-white flex flex-col fixed h-full z-20 shadow-xl">
      ...
      <main className="flex-1 ml-64 p-8 overflow-y-auto">
        ...
        {/* 1. DASHBOARD TAB (With Clickable Cards) */}
        {activeTab === 'dashboard' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {/* Card 1: Donations -> Goes to Donations Tab */}
            <div 
                onClick={() => setActiveTab('donations')}
                className="bg-emerald-500 rounded-2xl p-6 text-white shadow-lg cursor-pointer hover:scale-[1.02] transition-transform"
            >
                <div className="flex justify-between items-start">
                    <div>
                        <h3 className="mb-1 opacity-90">Total Donations</h3>
                        <p className="text-4xl font-bold">{stats.donations}</p>
                    </div>
                    <Heart className="opacity-20" size={48} />
                </div>
            </div>

            {/* Card 2: Volunteers -> Goes to Volunteers Tab */}
            <div 
                onClick={() => setActiveTab('volunteers')}
                className="bg-blue-500 rounded-2xl p-6 text-white shadow-lg cursor-pointer hover:scale-[1.02] transition-transform"
            >
                <div className="flex justify-between items-start">
                    <div>
                        <h3 className="mb-1 opacity-90">Total Volunteers</h3>
            ...
            {/* Card 3: Pending -> Goes to Donations Tab (filtered view logic can be added later) */}
            <div 
                onClick={() => setActiveTab('donations')}
                className="bg-amber-500 rounded-2xl p-6 text-white shadow-lg cursor-pointer hover:scale-[1.02] transition-transform"
            >
                <div className="flex justify-between items-start">
                    <div>
                        <h3 className="mb-1 opacity-90">Pending Requests</h3>
               ...
               <tbody className="divide-y divide-slate-100">
                 {volunteers.map((v) => (
                   <tr key={v.id} className="hover:bg-slate-50">
                     ...
                     <td className="p-4"><span className="bg-blue-50 text-blue-600 px-2 py-1 rounded text-xs font-bold">{v.interest || 'N/A'}</span></td>
                     <td className="p-4 truncate max-w-xs" title={v.message}>{v.message || '-'}</td>
            ...
            <div className="space-y-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-wrap items-center gap-4">
                    <div className="flex flex-col gap-1">
                        ...
                        <select 
                            className="border border-slate-300 rounded-lg px-4 py-2 bg-slate-50 font-medium outline-none focus:ring-2 focus:ring-blue-500"
                    ...
                    <div className="ml-auto">
                        <label className={`bg-blue-600 text-white px-6 py-2.5 rounded-lg cursor-pointer hover:bg-blue-700 transition flex items-center gap-2 font-bold shadow-md ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {galleryImages.map((img) => (
                        <div key={img.id} className="group relative bg-white rounded-xl shadow-sm border border-slate-200 h-64 flex items-center justify-center p-2">
                            <img 
                                src={img.image_path} 
                                alt="Gallery" 
        ...
        {activeTab === 'news' && (
            <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <form onSubmit={handlePostNews} className="space-y-4">
                        <div>
                            ...
                            <input 
                                ...
                                placeholder="E.g., Successfully distributed 500 kits"
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    {newsList.map(item => (
                        <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm flex gap-4 items-start">
                            <img src={item.image_path} alt="News" className="w-24 h-24 object-cover rounded-lg bg-slate-100 shrink-0" />
