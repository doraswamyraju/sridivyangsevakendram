...
const AdminDashboard = () => {
  ...
  // Gallery
  const handleUploadGallery = async (e) => { const file = e.target.files[0]; if (!file) return; const formData = new FormData(); formData.append('image', file); formData.append('program_id', selectedProgram); setUploading(true); await fetch(`${API_BASE_URL}/gallery.php`, { method: 'POST', body: formData }); fetchGallery(); setUploading(false); };
            ...
            {activeTab === 'gallery' && (
                <div className="space-y-6">
                    <div className="bg-white p-4 rounded-2xl shadow-sm flex flex-col md:flex-row items-center gap-4">
                        <select className="border p-2 rounded w-full md:w-auto bg-slate-50" value={selectedProgram} onChange={(e) => setSelectedProgram(e.target.value)}>{programs.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
                        <label className="bg-blue-600 text-white px-6 py-2 rounded cursor-pointer ml-auto w-full md:w-auto text-center">{uploading?"Uploading...":"Upload Photo"}<input type="file" className="hidden" accept="image/*" onChange={handleUploadGallery} disabled={uploading} /></label>
                    </div>
